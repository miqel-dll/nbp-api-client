...
export type GetTableDataForRelativeTime = GettingTableParamsEssentials & {
...
export type GetTableResponse<O, T extends TableCodes | TableCodeEnum, M extends `raw` | never = never> = [
    O
] extends [`json` | OutputFormatEnum.JSON] ? GetTableRow<T, M>[] : [O] extends [`xml` | OutputFormatEnum.XML] ? string : never;
...
export type GetRatesParams = GetCurrentRateParams | GetRatesTopCountParams | GetRatesTodayParams | GetRatesForDateParams | GetRatesForDateRangeParams | GetRatesForRelativeTimeParams;
export type GetCurrentRateParams = GettingRatesParamsEssentials & {
    mode: 'current' | GetTableDataEnum.CURRENT;
};
export type GetRatesTopCountParams = GettingRatesParamsEssentials & {
    mode: 'top-count' | GetTableDataEnum.TOP_COUNT;
    maxCount: number;
};
export type GetRatesTodayParams = GettingRatesParamsEssentials & {
    mode: 'today' | GetTableDataEnum.TODAY;
};
export type GetRatesForDateParams = GettingRatesParamsEssentials & {
    mode: 'date' | GetTableDataEnum.SPECIFIED_DATE;
    date: Date | string;
};
export type GetRatesForDateRangeParams = GettingRatesParamsEssentials & {
    mode: 'date-range' | GetTableDataEnum.BETWEEN_DATES;
    startDate: Date | string;
    endDate: Date | string;
};
export type GetRatesForRelativeTimeParams = GettingRatesParamsEssentials & {
    mode: 'days-before' | 'days-after' | GetTableDataEnum.DAYS_BEFORE | GetTableDataEnum.DAYS_AFTER;
    date: Date | string;
    days: number;
};
export type GetRatesResponse<O, T extends TableCodes | TableCodeEnum, M extends `raw` | never = never> = [
    O
] extends [`json` | OutputFormatEnum.JSON] ? GetRateRow<T, M> : [O] extends [`xml` | OutputFormatEnum.XML] ? string : never;
