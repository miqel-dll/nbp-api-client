...
describe('NBPApiClient', () => {
    ...
    describe('getGoldPriceJSON', () => {
        ...
        it('should throw error for unknown mode', async () => {
            await expect(client.getGoldPrice({ mode: 'unknown', currency: Iso4217CurrencyCodeEnum.PLN })).rejects.toThrow('Unknown retrieving mode for gold price.');
        });
    ...
    describe('getRates', () => {
        describe('Table A (mid rates)', () => {
            ...
            it('should throw error for unknown mode', async () => {
                await expect(client.getRates({ table: 'A', code: 'USD', mode: 'unknown' })).rejects.toThrow('Unknown retrieving mode for exchange rates.');
            });
        });
    ...
    describe('getTables', () => {
        describe('Table A (mid rates)', () => {
            ...
            it('should throw error for unknown mode', async () => {
                await expect(client.getTables({ table: 'A', mode: 'unknown' })).rejects.toThrow('Unknown retrieving mode for tables data for table.');
            });
        });
        describe('Table C (bid/ask rates)', () => {
            const mockResponseTableC = [
